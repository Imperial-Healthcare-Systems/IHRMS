...
const globalOtpState = globalThis as typeof globalThis & {
  __ihrmsOtpAttempts?: Map<string, AttemptState>
}

const otpAttempts = globalOtpState.__ihrmsOtpAttempts ?? new Map<string, AttemptState>()
globalOtpState.__ihrmsOtpAttempts = otpAttempts

function getAuthSecret() {
  const secret = process.env.NEXTAUTH_SECRET ?? process.env.AUTH_SECRET

  if (!secret) {
    throw new Error('Missing NEXTAUTH_SECRET or AUTH_SECRET for OTP authentication.')
  }
...
export function verifyOtpChallenge({ email, otp, challengeToken }: OtpVerificationInput): OtpVerificationResult {
  try {
    if (!/^\d{6}$/.test(otp)) {
      return { valid: false, error: 'Enter a valid 6-digit OTP.' }
    }

    const [encodedPayload, signature] = challengeToken.split('.')

    if (!encodedPayload || !signature) {
      return { valid: false, error: 'Invalid OTP challenge.' }
    }
    ...
    if (payload.v !== 1) {
      return { valid: false, error: 'Unsupported OTP challenge.' }
    }
